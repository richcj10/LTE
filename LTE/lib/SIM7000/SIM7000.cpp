...
boolean Adafruit_FONA::setAudio(uint8_t a) {
  return sendCheckReply(F("AT+CSDVC="), a, ok_reply);
}
...
boolean Adafruit_FONA::getGPS(float *lat, float *lon, float *speed_kph, float *heading, float *altitude,
  ...
  if (_type == SIM808_V2 || _type >= SIM7000) {
    ...
    if (onoff) {
      ...
      // if (_type < SIM7000) { // UNCOMMENT FOR LTE ONLY!
        ...
        if (! sendCheckReply(F("AT+CIICR"), ok_reply, 10000))
          return false;
        if (! sendCheckReply(F("AT+CIFSR"), ok_reply, 10000))
          return false;
      // } // UNCOMMENT FOR LTE ONLY!
...
boolean Adafruit_FONA::postData(const char *request_type, const char *URL, const char *body, const char *token, uint32_t bodylen) {
  ...
  if (request_type == "GET") {
  if (! sendCheckReply(F("AT+HTTPACTION=0"), ok_reply, 10000)) //ACTION 0 = READ
    return false;
  }
  else if (request_type == "POST" && bodylen > 0 ) { // POST with content body
  //*************
  // CHAGES MADE IN THIS SECTION
  // *****************
...
boolean Adafruit_FONA_LTE::HTTP_POST(const char *URI,const char *body, uint8_t bodylen, uint16_t Timeout) {
  ...
  delay(100);
  int Result = sprintf(cmdBuff, "AT+SHBOD=\"%s\",%i", body, bodylen);
